...
| v2.0 | 数字人 | 🟡 开发样例完成（内置样例引擎 + http 适配样例 + 前端面板；正式接入哪家由使用者决定） |
...
| 横向扩展（多副本） | ⬜ 需先换共享存储 |
| 真实模型链路压测与 Prompt 调优 | 🟡 A4 无来源主张已收紧、A3/A5/A6/A11 输出结构已压缩（离线回归全绿）；真实网关复测待跑 |
...
| 3 | **用真实网关数据复测 Prompt 与预算** | 已跑通真实链路（DeepSeek）并量到首个基线：单任务约 **315s / 7.9 万 token / $0.04 / 质量分 62**；瓶颈是模型输出冗长与 A4 生成无来源主张导致 A6 否决 | 代码侧已完成：A4 提示词收紧「无来源数据一律不写」，A3/A5/A6/A11 增加输出体量控制（离线回归全绿）；**下一步是真实网关复测**，按复测结果继续调 |
...
| 指标 | 实测值 | 说明 |
| --- | --- | --- |
| 单任务墙钟 | ~315s | 11 次模型调用，单次 17–40s |
| token | prompt ~9.6k / completion ~70k | completion 远大于 prompt：结构化输出 + 模型推理 |
| 成本 | ~$0.04 / 篇 | 按 2026-09-10 定价（缓存命中输入 ¥0.02/M） |
| 质量分 | 62 | A6 事实核查否决「无来源主张」8 项 —— **门禁按设计生效** |
...
| # | 事项 | 为什么需要人 |
|---|---|---|
| 9 | **选定数字人服务商并决定是否正式接入** | 系统提供的是接入样例（内置引擎 + http 适配）；正式接入需明确产品形态并选型服务商，网关契约见 `DIGITAL_HUMAN_API_URL` 说明 |
