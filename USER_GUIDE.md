...
新建任务、动效工场与运行时设置；任务详情含基本信息、质量评分卡、审批横幅、
发布回填与数字人渲染面板，以及下述六个标签页。
...
以上门禁可一键执行：`make verify`（构建 + 六道关卡，与 CI 同序）、`make release`（发行版自检）、`make help` 看全部目标；Windows 直接用根目录 `make.bat`。除 `run` / `dev` 外的目标自动注入离线引擎（`LLM_PROVIDER=mock`），不会烧真实 token；发行版版本号在根目录 `VERSION`（`make tag` 打 `v<版本>` 标签）。
