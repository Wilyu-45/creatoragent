"""行业洞察库与爆款案例库（移植自 server/knowledge/industry.ts）。
...
DEFAULT_INDUSTRY: IndustryProfile = INDUSTRY_PROFILES["消费品"]


def industry_profile(industry: str) -> IndustryProfile:
...
def channel_rule(channel: str) -> ChannelRule:
    if channel in CHANNEL_RULES:
        return CHANNEL_RULES[channel]
    for key, rule in CHANNEL_RULES.items():
        if key in channel:
            return rule
    return CHANNEL_RULES["小红书"]


#: 渠道 → 标题字数上限（信息流截断阈值，A5 审校与 A9 适配共用）。
...
def title_limit(channel: str) -> int:
    if channel in CHANNEL_TITLE_LIMIT:
        return CHANNEL_TITLE_LIMIT[channel]
    for key, limit in CHANNEL_TITLE_LIMIT.items():
        if key in channel or channel in key:
            return limit
    return CHANNEL_TITLE_LIMIT["小红书"]
...
def cases_for(industry: str, channel: str) -> list[CaseEntry]:
    def score(entry: CaseEntry) -> int:
        value = 0
        if entry.industry == industry:
            value += 3
        elif entry.industry in industry or industry in entry.industry:
            value += 1
        if entry.channel == channel:
            value += 2
        return value

    ranked = sorted(CASE_LIBRARY, key=score, reverse=True)
    return ranked[:3]


# ------------------------------------------------------------------ #
# SEO 与渠道适配规则（服务 A9 渠道运营与 SEO）                          #
# ------------------------------------------------------------------ #
...
SEO_PATTERNS: dict[str, SeoPattern] = {
    "小红书": SeoPattern(
        search_intent="决策前搜口碑：用户搜「值不值 / 好不好 / 测评」做购买前验证",
        modifiers=["{kw}测评", "{kw}真实体验", "{kw}怎么选", "{kw}值得买吗", "{kw}踩坑"],
        placement=["标题前 12 字必含主关键词", "正文首段自然出现一次", "文末话题标签覆盖垂类词"],
        difficulty="medium",
        notes=["平台搜索权重集中在标题与话题标签", "同一关键词连续堆砌会被判定为营销内容"],
    ),
    "抖音": SeoPattern(
        search_intent="兴趣触发为主、搜索为辅：靠推荐流拦截，搜索承接品牌词",
        modifiers=["{kw}推荐", "{kw}开箱", "{kw}对比", "{kw}教程"],
        placement=["口播前 3 秒说出关键词", "字幕与画面文字同步强化", "话题标签紧跟平台热点"],
        difficulty="high",
        notes=["搜索流量占比低，优先完播率与互动率", "关键词需口语化，避免书面术语"],
    ),
    "公众号": SeoPattern(
        search_intent="社交传播与深度阅读：靠转发与在看扩散，搜一搜承接长尾",
        modifiers=["{kw}深度解读", "{kw}方法论", "{kw}复盘", "{kw}趋势"],
        placement=["标题使用「主关键词：价值点」冒号结构", "导语段复现关键词", "文末标签用于搜一搜聚合"],
        difficulty="low",
        notes=["标题信息量优先于关键词密度", "数据引用需标注来源以维持可信度"],
    ),
    "知乎": SeoPattern(
        search_intent="问题导向检索：用户搜「为什么 / 怎么做 / 哪个好」寻找专业答案",
        modifiers=["{kw}哪个好", "为什么{kw}", "{kw}怎么做", "{kw}原理", "{kw}避坑"],
        placement=["问题式标题直接嵌入关键词", "结论段前置关键词", "绑定 2-4 个话题"],
        difficulty="medium",
        notes=["长尾搜索流量占比高，答案需可被引用", "专业领域需披露利益相关"],
    ),
    "电商详情页": SeoPattern(
        search_intent="站内搜索与类目筛选：关键词决定能否被搜到",
        modifiers=["{kw}官方旗舰店", "{kw}正品", "{kw}大容量", "{kw}礼盒装"],
        placement=["首屏主图利益点含主关键词", "卖点标题动宾结构嵌入属性词", "参数表覆盖规格长尾词"],
        difficulty="high",
        notes=["标题关键词需与类目属性一致", "禁止使用极限词与虚假原价"],
    ),
    "官网": SeoPattern(
        search_intent="品牌词与解决方案词检索：用户带着明确意图直达",
        modifiers=["{kw}解决方案", "{kw}价格", "{kw}对比", "{kw}免费试用"],
        placement=["主标题含品牌词 + 品类词", "首屏副标题补充场景词", "落地页与内页做内链锚文本"],
        difficulty="low",
        notes=["长尾靠落地页矩阵承接", "需保证首屏 3 秒可读"],
    ),
    "PR稿": SeoPattern(
        search_intent="媒体转载与信息核实：搜索用于查证事件与主体",
        modifiers=["{kw}发布", "{kw}合作", "{kw}最新消息"],
        placement=["标题含主体与事件关键词", "导语 5W 完整", "关于我们段补充品牌词"],
        difficulty="low",
        notes=["不追求关键词密度，追求可被引用", "数据需可核实"],
    ),
}

DEFAULT_SEO_PATTERN: SeoPattern = SEO_PATTERNS["小红书"]


def seo_pattern(channel: str) -> SeoPattern:
    if channel in SEO_PATTERNS:
        return SEO_PATTERNS[channel]
    for key, pattern in SEO_PATTERNS.items():
        if key in channel or channel in key:
            return pattern
    return DEFAULT_SEO_PATTERN


#: 各渠道建议发布时段（面向目标人群活跃峰值的通用经验值）。
CHANNEL_PUBLISH_SLOTS: dict[str, list[str]] = {
    "小红书": ["07:30-08:30 通勤时段", "12:00-13:00 午休", "20:00-22:30 睡前高峰"],
    "抖音": ["07:00-09:00 通勤", "12:00-14:00 午休", "19:00-23:00 黄金档"],
    "公众号": ["07:00-08:00 早读", "20:00-22:00 睡前阅读"],
    "知乎": ["12:00-13:30 午休", "21:00-23:00 深度阅读"],
    "电商详情页": ["10:00-11:00", "20:00-22:00 大促活跃期"],
    "官网": ["工作日 10:00-11:30", "工作日 14:00-16:00"],
    "PR稿": ["工作日 09:30-11:00 媒体工作时间"],
}


def publish_slots(channel: str) -> list[str]:
    if channel in CHANNEL_PUBLISH_SLOTS:
        return CHANNEL_PUBLISH_SLOTS[channel]
    for key, slots in CHANNEL_PUBLISH_SLOTS.items():
        if key in channel or channel in key:
            return slots
    return CHANNEL_PUBLISH_SLOTS["小红书"]
