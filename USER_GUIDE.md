...
布局为「顶栏 + 左侧任务列表 + 右侧任务详情」：顶栏放服务状态、任务计数、OTLP 状态、
新建任务、动效工场与运行时设置；任务详情含基本信息、质量评分卡、审批横幅、发布回填与数字人渲染面板，以及下述六个标签页。
...
- **动效工场**：视频动效组件库试演与时间轴编排（纯前端工具，与任务数据无关），见 [12.5](#125-动效工场视频动效组件库)。
- **运行时设置**：全部运行参数集中于此（分组清单见 [6.1](#61-界面配置即时生效无需重启)）。
...
| 分组 | 配置项 |
| --- | --- |
| 模型 | 提供方（mock / openai）、模型名、Base URL（含本地端点一键填入）、API Key、Temperature、Max Tokens、超时、多模态输入开关与图片上限、深度思考 |
| 智能体模型覆盖 | 每个智能体可单独覆盖 模型 / Base URL / API Key（留空继承全局） |
| 编排与门禁 | Turn Budget、最大返工轮次、质量分阈值、素材研读调用上限、自动审批 |
| 成本与缓存 | 单任务成本上限（USD）、单任务 token 上限、是否启用响应缓存 |
| 向量检索 | 提供方（local / openai）、Base URL、模型、维度、语义权重（0–1）、API Key |
| 发布投递 | webhook 地址、失败重试次数、是否自动投递 |
| 质量评估 | 介入方式（off / advisory / blocking）、评估器（offline / llm）、评估模型、通过线、权重 |
| 联网检索 / 站点监控 | 检索网关（provider / URL / Key / 条数 / 超时 / 抓页）、站点 URL 列表 |
| OTLP 与数字人 | 端点 / 服务名 / 采样器与比例 / 鉴权头；数字人 provider / 网关 URL / 形象 / 超时 / Key |
| 服务监听 | host / 端口（**重启后生效**，其余修改均即时生效） |
| 访问令牌 | 本地保存 API Token（随请求带 `X-API-Token`） |
| 知识库 / 记忆库 | 只读浏览与检索 |

> 界面保存值持久化到 `data/settings.json`；优先级：**进程环境变量 > 界面保存值 > `.env` > 内置默认**（CI 注入的 `LLM_PROVIDER=mock` 即压过界面保存值）。

> 模型提供方选 `openai` 时兼容一切 OpenAI 协议网关；本地模型一键填端点（Ollama `:11434/v1`、LM Studio `:1234/v1`、vLLM `:8000/v1`，
> 须带 `http://` 前缀；本机 / 私网端点**不计费**、token 照记）。API Key 回显为掩码，明文不落盘。

### 6.2 环境变量

可在根目录 `.env`、进程环境变量或界面「运行时设置」中设置（优先级见 [6.1](#61-界面配置即时生效无需重启)）。**本表是配置项的唯一归属处**，完整默认值以 [`.env.example`](.env.example) 为准：
...
默认**不需要**任何外部服务；想接真实追踪后端时，在界面「运行时设置 → 调用轨迹」或环境变量里配置端点即可：
...
**它是「原生创作」，不是翻译**：选定非中文语言后，智能体收到本地化指令——用目标语言原生创作（不先写中文再翻译）、遵守当地表达惯例与合规红线、按目标语言字数口径控制标题。
...
界面「运行时设置 → 知识库」可看各语言合规覆盖；**记忆库按语言隔离**（英文资产不会被中文任务当作品牌调性基线复用，反之亦然）。
...
- 形态参数按渠道给（抖音 45s/9:16/5 镜、视频号 60s、B 站 120s/16:9、TikTok 30s…），时间轴由知识层骨架生成，**保证单调且总时长与目标一致**。
...
> **定位**：数字人渲染**不在本系统内实现**（各家授权、形象库、计费与回调协议差异极大，接哪家属产品形态决策）；系统提供可回归的接入样例（`app/core/digital_human.py`）。

- **前置条件**：任务已产出 `video_script` 产物；没有脚本时创建接口返回 409。
- **两条样例通道**（创建作业时可用 `provider` 覆盖）：`sample`（默认，离线确定性模拟并按脚本生成渲染清单）/ `http`（在界面「运行时设置 → 数字人渲染」或环境变量 `DIGITAL_HUMAN_API_URL` 配置网关后，对接「POST 建任务 → GET 查状态」最小契约，未配置时**显式失败**）。
- **界面**：任务详情出现「数字人渲染」面板（创建作业、进度条、渲染清单分镜表、成片地址）。
- **生命周期**：状态在读取时惰性推进（不靠后台线程）；任务删除时渲染作业一并回收；按租户隔离。

### 12.5 动效工场（视频动效组件库）

顶栏「✦ 动效工场」进入。纯前端工具，把 [12.3](#123-视频脚本) 产出的分镜画面设计成可预览的动效序列：

- **预设试演**：22 个动效预设按 入场 / 强调 / 出场 / 转场 分组（FadeIn、Typewriter、HighlightSweep、Glitch…），选中即自动播放。
- **JSON 脚本**：声明式时间轴 `{duration, tracks:[{preset, at, duration, hold, text, easing, params, style}]}`，「应用到预览」整体预演，非法字段以中文报错；内置示例一键载入。
- **播放控制**：播放/暂停、进度拖动、0.5–2× 倍速、循环；**播完后出现「重播 / 回到起点」覆盖层**，重播时全部动效从头播放（状态完整重置）。
- **实现**：纯 React 时间驱动渲染（`src/motion/`，无内部定时器），seek / 倍速天然免费；将来接渲染服务即可导出 MP4。

### 12.6 视频工场（字幕 → 成片）

顶栏「▶ 视频工场」进入。纯前端字幕成片工具（`src/vg/`）：粘贴字幕文件，自动建轴、实时预览、导出真实视频文件。

- **字幕**：SRT / WebVTT / ASS / 纯文本（按标点自动断句、按语速计时），解析后即时显示条数与总时长。
- **自动建轴**：智能分幕 + 节拍点卡点吸附（偏差 0，满足 ±0.1s）；6 主题 × 4 背景、7 档调色、6 类转场、6 种字幕动效、Ken Burns 推拉、引号/数字强调，每项可从「智能轮换」改为固定。
- **去 AI 味**：种子随机轮换动效避免机械重复，「🎲 换一批」一键重随机；内置 AI 味词 lint（「首先/其次」式开头会提示改写）。
- **音频**：可挂 BGM（循环 + 淡入淡出）与旁白，旁白出现时 BGM 自动压低（ducking）。
- **导出**：Canvas 实时编码 720p/1080p/4K、24/30/60 fps，MP4 优先、WebM 回退；进度/暂停/继续/取消齐全；耗时 ≈ 视频时长，导出期间需保持页面前台（切后台自动暂停）。
...
以上门禁可一键执行：`make verify`（构建 + 六道关卡，与 CI 同序）、`make release`（发行版自检）、`make help` 看全部目标；Windows 直接用根目录 `make.bat`。除 `run` / `dev` / `app` 外的目标自动注入离线引擎（`LLM_PROVIDER=mock`），不会烧真实 token；发行版版本号在根目录 `VERSION`（`make tag` 打 `v<版本>` 标签）。

**桌面版发行物**：`make app` 打包单文件 exe 到 `release/CreatorStudio.exe`（内嵌前端，PyInstaller 需 `pip install pyinstaller`）。双击启动即在浏览器打开界面，数据落在 exe 旁 `data/`；可选在 exe 旁放 `.env`。源码模式等价入口 `python -m app.desktop`。

`doctor.py` 覆盖离线引擎、记忆库 RAG 与租户隔离、向量检索、发布排期、鉴权、检查点后端、评估器、回归判定器、
调用轨迹与 OTLP、多语言、视频脚本、多模态与计算沙箱等；`smoke_api.py` 覆盖主流程、自动审批、记忆与发布闭环、
评估流水线、错误分支等。**各自的确切覆盖范围以脚本输出清单为准**，本文不复述。

**持续集成**：`.github/workflows/ci.yml` 把上述关卡接成自动化门禁（**无需任何模型密钥**，默认走离线引擎）：
后端六关卡 → 前端 `typecheck` + `build` → 文档一致性关卡；失败时自动上传 `.doctor-data/**/server.log` 诊断。
